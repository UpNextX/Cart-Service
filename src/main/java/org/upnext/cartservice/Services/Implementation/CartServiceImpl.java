...
public class CartServiceImpl implements CartService {
    ...
    public CartDto addItemToCart(Long id, CartItemRequest cartItemRequest) {
        ...
        ProductDto productDto = productsClient.getProduct(cartItemRequest.getId());

        Optional< CartItem> existingItemOpt = cart.getItems().stream()
                .filter(item -> item.getProductId().equals(cartItemRequest.getId()))
                .findFirst();
        ...
        if(existingItemOpt.isPresent()){
            ...
            cartItem.setProductId(cartItemRequest.getId());
